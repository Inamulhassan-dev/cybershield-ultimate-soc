...
// Autonomous Radar Polling
// Guarantees the radar populates even if the HTML cache failed to update the classifyTraffic() function
async function autoFetchRadarBlips() {
    try {
        const res = await fetch('/api/traffic-data');
        const data = await res.json();
        if (window.updateRadarBlips && data.traffic) {
            window.updateRadarBlips(data.traffic);
        }
    } catch(e) {}
}

// Start polling radar data independently every 5 seconds
setInterval(autoFetchRadarBlips, 5000);
autoFetchRadarBlips();
